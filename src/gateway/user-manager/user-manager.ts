...
export default class UserManager {
  ...
  removeUser(socketId: string) {
    this.users.splice(this.users.findIndex((existingUser) => existingUser.socketId === socketId), 1);
  }

  private checkIfUserExist(user: User): User | null {
    const checkUser = this.users.find((existingUser) => existingUser.id === user.id);
